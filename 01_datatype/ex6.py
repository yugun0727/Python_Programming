...
#  파이썬 문자열의 중요한 특징
# =========================================================
# 1. 문자열은 (    원본불변 ) 이다 -> (     imutable  ) (문자열 연산 시 새로운 문자열이 만들어짐)
# 2. 문자열은 (   반복 가능  ) 하다 -> (       ) (반복처리 가능)
# 3. 문자열은 ( 순서  )가 있다 -> (   python    ) (인덱싱, 슬라이싱 가능)
...
# # Iterator에 의해 반복 처리
it = iter(s)   # 문자열(Iterable)에 iter()를 호출하면 Iterator 객체가 생성
